...
### 🌊 Gravitational Waves & Virtual LIGO Strain Detector (<kbd>J</kbd>)
- **Einstein Quadrupole Metric Tensor Perturbations**: High-precision evaluation of dynamic gravitational wave metric ripples generated by accelerating non-axisymmetric mass quadrupole moments:
  $$h_{ij}^{\text{TT}}(t, \vec{x}) = \frac{2G}{c^4 r} \left[ \ddot{I}_{ij}\left(t - \frac{r}{c}\right) \right]^{\text{TT}}$$
- **Independent Waveform Visualization**: Gravitational wave ripples can be toggled on/off **independently** from the 3D Spacetime potential curvature grid (<kbd>J</kbd> / toolbar `[GW]` / `#tog-gwwaves`). Allows viewing pure gravitational radiation undulating around $0.0$ with positive crests and negative troughs.
- **Propagating Merger & Collision Bursts**: High-energy celestial collisions, hyper-velocity mergers, and relativistic binary inspirals emit transient spherical wavefront pulses that propagate radially outward at the speed of light ($c$).
- **Live LIGO Interferometer Telemetry**: Real-time HUD reporting:
  - **Dimensionless Strain Amplitude ($h$)**: Calibrated to authentic astrophysical sensitivities ($10^{-21}$ strain units).
  - **Quadrupole Frequency ($f_{\text{GW}}$)**: Dynamic gravitational wave frequency in Hertz.
  - **Radiated Gravitational Power ($P_{\text{GW}}$)**: Radiated energy rate in Watts ($dE/dt \propto \dddot{I}^2$).

### 🛰️ 2D Tactical Viewport, 2D Offload Mode & Continuous Gravitational Heatmap
- **2D Main Offload Mode (<kbd>Z</kbd> / <kbd>F5</kbd>)**: Completely suspends the 3D rendering pipeline and promotes the 2D Tactical Viewport to the primary display. Perfect for battery-efficient mobile laptops and low-power hardware while running the full 64-bit multi-substep N-body physics engine. Includes full 2D slingshot spawning, target tracking (<kbd>T</kbd>), focus (<kbd>F</kbd>), and telemetry HUD.
- **Configurable 2D Heatmap Resolution (<kbd>Shift</kbd>+<kbd>H</kbd> / <kbd>Alt</kbd>+<kbd>H</kbd> or <kbd>Alt</kbd>+<kbd>[</kbd> / <kbd>Alt</kbd>+<kbd>]</kbd>)**: Step 2D Heatmap resolution up or down on demand across 10 progressive tiers (`16, 24, 32, 48, 64, 80, 96, 128, 160, 200` columns). Dynamically scales vertical rows to preserve **perfect square pixel cells** and exact aspect ratios across all docked and fullscreen window dimensions.
- **On-Demand 3D Spacetime Resolution Stepping (<kbd>{</kbd> / <kbd>}</kbd> / 2D Toolbar `[RES-]`/`[RES+]`)**: Dynamically scale grid subdivisions across 11 tiers (`32, 48, 64, 80, 96, 120, 160, 200, 256, 384, 512` vertices). In the 2D Viewport, `[RES-]`/`[RES+]` intelligently steps 2D Heatmap resolution when Heatmap is active (glowing amber) and Spacetime grid when inactive (or with <kbd>Shift</kbd>).
- **Tactical Object Icons vs Clear Circles (<kbd>Y</kbd> / `[ICON]`)**: Renders rich tactical astronomical glyphs for stars, black holes, pulsars, terrestrial worlds, gas giants with rings, desert worlds, ice giants, moons, and scientific probes. **Clean Circles Mode** hides all textures and pictures, leaving pure geometric outlines.
- **2D Body Circles & Center Dots Toggle (<kbd>Shift</kbd>+<kbd>C</kbd> / `[CIR]`)**: Unified toggle for both the outer body outline circle and pinpoint center core dot. When toggled off, both disappear cleanly.
- **2D Direction Vector Indicators (<kbd>V</kbd> / `[VEC]`)**: Velocity and direction indicators rendered in 2D matching the 3D vector visualizer.
- **2D Orbit Reference Trails (<kbd>Shift</kbd>+<kbd>T</kbd> / `[TRL]`)**: Toggle orbit history paths in 2D independently or alongside 3D trails.
- **Zero-Overlap Adaptive 2-Row Toolbar**: Automatically transitions between a compact 2-row layout in docked mode (< 700px) and a sleek single-row layout in fullscreen/offload mode (>= 700px), with zero button overlap or clipping across all screen resolutions (1024x768 to 2560x1440).
- **Non-Obstructing HUD Layout (<kbd>I</kbd> / <kbd>F11</kbd> / `[HUD]`)**: Real-time FPS and telemetry HUD overlay is decoupled and toggleable, guaranteeing the top of the 2D viewport is never blocked or obscured.
- **Orthographic Orbital Minimap & Fullscreen View (<kbd>M</kbd>)**: High-clarity 2D tactical view projecting planetary orbits and mass distribution onto the ecliptic plane. Docked on the **right side** — automatically shifts left of the Inspector panel when a body is selected, guaranteeing **zero UI overlap**.
- **Toggleable 2D Map Labels (<kbd>K</kbd>)**: Toggle body name tags and Lagrange point annotations on/off. **Clean Mode** hides all text, leaving pure orbital circles, velocity vectors, and crosshairs.
...
- **3D Gravitational Heatmap Plane Grid** (Bottom Bar `3D Heatmap` toggle): Renders a thermodynamic potential color gradient directly on the **Y=0 orbital plane** in 3D space.
...
| <kbd>J</kbd> | Toggle **Gravitational Waves & LIGO Detector** (metric quadrupole ripples & live strain HUD) |
| <kbd>Z</kbd> / <kbd>F5</kbd> | Toggle **2D Main Offload Mode** (suspends 3D rendering, runs 2D viewport as primary display) |
| <kbd>{</kbd> / <kbd>}</kbd> *(Shift+`[` / `]`)* | Decrease / Increase **Spacetime Grid Resolution** (32x32 up to 512x512) |
| <kbd>Shift</kbd>+<kbd>H</kbd> / <kbd>Alt</kbd>+<kbd>H</kbd> *(or Alt+`[` / `]`)* | Step **2D Heatmap Viewport Resolution** (16 up to 200 columns) |
...
| <kbd>Y</kbd> *(or Toolbar `[ICON]`)* | Toggle **2D Celestial Object Icons / Clear Circles Mode** (tactical glyphs vs pure outlines) |
| <kbd>Shift</kbd>+<kbd>C</kbd> *(or Toolbar `[CIR]`)* | Toggle **2D Body Circles & Center Dots** on / off |
| <kbd>Shift</kbd>+<kbd>T</kbd> *(or Toolbar `[TRL]`)* | Toggle **Orbit Reference Trails** on / off |
| <kbd>V</kbd> *(or Toolbar `[VEC]`)* | Toggle **Object Velocity & Direction Vectors** on / off |
| <kbd>I</kbd> | Toggle **Integrator Algorithm** (4th-Order Symplectic Yoshida $O(\Delta t^4)$ $\leftrightarrow$ Velocity Verlet) |
| <kbd>F11</kbd> *(or Toolbar `[HUD]`)* | Toggle **Simulation Telemetry & Performance HUD** overlay |
...
| <kbd>O</kbd> | Toggle **Gravitational 3D Vector Field** (32x32 needle grid) |
...
### 9. Relativistic Gravitational Waves & Quadrupole Radiation
In linearized General Relativity, accelerated mass distributions produce transverse-traceless metric perturbations $h_{ij}^{\text{TT}}$ propagating at the speed of gravity $c$:

The reduced quadrupole moment tensor of the mass distribution is:

$$I_{ij} = \sum_{k} m_k \left( x_k^i x_k^j - \frac{1}{3}\delta_{ij} r_k^2 \right)$$

Its second time derivative (quadrupole acceleration) drives the asymptotic strain amplitude at distance $r$ with retarded time $t_{\text{ret}} = t - r/c$:

$$h_{ij}^{\text{TT}}(t, \vec{r}) = \frac{2G}{c^4 r} \ddot{I}_{ij}\left(t - \frac{r}{c}\right)$$

The total radiated gravitational wave power (Einstein's Quadrupole Formula) is:

$$P_{\text{GW}} = -\frac{dE}{dt} = \frac{G}{5c^5} \sum_{i,j} \langle \dddot{I}_{ij} \dddot{I}_{ij} \rangle$$

- **Concentric Wavefront Propagation**: Metric ripples radiate outward spherically at coordinate propagation speed $c_{\text{gw}}$, decaying with distance as $1/r$.
- **Real-Time Virtual LIGO Telemetry**: An in-engine interferometer detector samples the local metric perturbation $h(t) \sim \Delta L / L$, reporting instantaneous peak strain, dominant frequency $f_{\text{GW}}$, and equivalent solar luminosity ($L_\odot$) on the live HUD.

### 10. Inelastic Collision Merging & Swarm Optimization ($N > 250$)
When physical radii of two celestial bodies overlap ($\|\vec{r}_1 - \vec{r}_2\| \le R_1 + R_2$), a completely inelastic merger occurs:

- **Mass Conservation**:
  $$M_{\text{merged}} = m_1 + m_2$$
- **Linear Momentum Conservation**:
  $$\vec{v}_{\text{merged}} = \frac{m_1 \vec{v}_1 + m_2 \vec{v}_2}{m_1 + m_2}$$
- **Volume & Density Conservation**:
  Assuming constant mean density $\rho$, the consolidated radius follows spherical volume conservation:
  $$R_{\text{merged}} = \left( R_1^3 + R_2^3 \right)^{1/3}$$
- **Center of Mass Coalescence**:
  $$\vec{r}_{\text{merged}} = \frac{m_1 \vec{r}_1 + m_2 \vec{r}_2}{m_1 + m_2}$$
- **Swarm Collision Threshold ($N > 250$)**:
  In dense cosmic swarms (e.g. Gravitational Collapse 1,600 bodies, Milky Way 10K), computing $O(N^2)$ pairwise radii checks every substep incurs over $1.28 \times 10^6$ distance calculations per substep. To guarantee 144 FPS throughput, pairwise collision checks between minor debris/cloud particles ($m < 20.0$, $R < 1.5$, neither being a Black Hole or Pulsar) are gracefully bypassed when $N > 250$. Major-body accretions (e.g. stars colliding or black holes feeding) remain active. This allows cosmic dust clouds to undergo collisionless kinetic relaxation and realistic virialization into stable spherical clusters rather than collapsing prematurely into a single point mass.
...
├── gravitational_waves.go   # Relativistic quadrupole wave radiation, metric ripples & LIGO detector
...
│   │   ├── gravitational_waves.go # WASM gravitational wave quadrupole generator & LIGO strain
│   │   ├── lagrange.go            # WASM CR3BP analytical solver & Jacobi potential
│   │   ├── kepler.go              # WASM Keplerian orbital mechanics & presets
│   │   ├── physics.go             # WASM Verlet/Yoshida integrators, Barnes-Hut & collisions
│   │   ├── octree.go              # WASM spatial octree implementation
│   │   ├── math3d.go              # WASM vector math & matrix utilities
│   │   ├── presets.go             # WASM astronomical presets & cosmic scenes
│   │   ├── types.go               # WASM engine data structures & bridge models
│   │   └── main.go                # WASM entry point & JavaScript bridge bindings
...
├── gpu_test.go              # GPU compute shader and SSBO buffer unit tests
...
- [x] **Relativistic Gravitational Waves & Virtual LIGO Strain Detector**: Quadrupole radiation ripples propagating across spacetime with real-time waveform strain ($h \sim \Delta L/L$), frequency, and peak luminosity HUD.
- [x] **2D Main Offload Mode**: Low-power / laptop execution mode suspending the 3D rendering pipeline to drive the simulation exclusively through the high-performance 2D tactical viewport with full interactive spawning, tracking, and telemetry.
- [x] **Dynamic Dual-Resolution Stepping**: Real-time scalable 3D spacetime grid (32x32 to 512x512) and 2D continuous heatmap (16 to 200 cols) with aspect-ratio-preserving square cells.
- [x] **Tactical 2D Iconography & Clean Display Modes**: Customizable tactical celestial glyphs vs geometric outlines, unified body circle/dot visibility toggles, and velocity direction vectors.
